...
- Enforce the required statuses listed in `tools/policy/branch-required-checks.json` (contract source of truth). At
  ...
  `hook-parity (ubuntu-latest)`, `vi-history-scenarios-linux`, and `commit-integrity / commit-integrity`.
...
- One-time GitHub CLI snippet (admin only):
  ...
  gh api repos/$REPO/branches/develop/protection \
    ...
    -f required_status_checks.contexts[]='commit-integrity / commit-integrity' \
