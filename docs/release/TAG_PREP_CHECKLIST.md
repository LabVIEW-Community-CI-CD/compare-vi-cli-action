...
- [ ] `CHANGELOG.md` contains a finalized
      `## [v0.6.10] - 2026-04-01` section.
- [ ] Stable docs reference `v0.6.9` consistently until `v0.6.10` publication
...
```bash
pwsh -NoLogo -NoProfile -File tools/Test-CompareVIHistoryBundleCertification.ps1 -BundleArchivePath tests/results/_agent/bundle-fix/artifacts/CompareVI.Tools-v0.6.10.zip -ResultsDir tests/results/_agent/bundle-fix/certification -SummaryJsonPath tests/results/_agent/bundle-fix/certification/summary.json
```
...
- [ ] `README.md` and `docs/USAGE_GUIDE.md` still treat `v0.6.9` as the
...
```pwsh
git tag -a v0.6.10 -m "v0.6.10: promote Windows NI proof authority and local autonomy"
```
...
- [ ] Notify consumers that `v0.6.10` supersedes `v0.6.9` as the supported
      stable pin.
