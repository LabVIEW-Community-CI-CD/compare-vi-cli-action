...
6. **Validate schema** - optional: `./tools/npm/bin/npm run schema:validate -- --schema docs/schema/generated/lvcompare-capture.schema.json --data tests/results/teststand-cli/compare/lvcompare-capture.json` to ensure the capture passes the new schema.
