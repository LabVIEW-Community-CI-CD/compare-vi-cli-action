...
available even when fresh harness artefacts are not present. `./tools/npm/bin/npm run session:teststand:validate` now checks both the sample
...
Validate the session index with `./tools/npm/bin/npm run session:teststand:validate` so schema regressions surface immediately when the harness
outputs change.
...
| Command | Purpose |
| --- | --- |
| `./tools/npm/bin/npm run tests:discover` | Ensure the TypeScript manifest is current before mapping tests to harness inputs. |
