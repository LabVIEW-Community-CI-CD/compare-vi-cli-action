# Template Docker Profile Requirement V5
...
- `TDPR-031` Each Docker-profile render shall preserve the required
  `vi-history` surfaces.
- `TDPR-031A` Template self-validation shall verify `TDPR-031` for at least
  one `docker` render and at least one `mixed` render.
