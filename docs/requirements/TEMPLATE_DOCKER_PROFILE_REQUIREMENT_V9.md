# Template Docker Profile Requirement V9
...
| Producer implementation content | Files or workflows owned by the Producer that implement queue orchestration, governor control, capability publication, image publication, or release/publication behavior |
| consumer-facing outputs | Files, workflows, policies, manifests, entrypoints, and documentation surfaces rendered for a Generated repository to consume upstream contracts |
...
- `TDPR-011` The Docker-profile render shall limit descendant-facing contract
  ...
  surfaces.
- `TDPR-012` The Docker-profile render shall not include Producer
...
- `TDPR-024` The Template shall remain limited to distribution, cookiecutter
  stamping, and rendering the consumer-facing outputs required by `TDPR-005`,
  `TDPR-005A`, `TDPR-006`, `TDPR-007`, `TDPR-009`, `TDPR-010`, and `TDPR-031`.
...
- `TDPR-025A` A Generated repository rendered from the Template shall not
  vendor Producer implementation content.
...
- `TDPR-029` Template self-validation shall verify that `docker` and `mixed`
  renders satisfy `TDPR-012` and `TDPR-025A`.
...
- `Cross-Repo VI History Capture` — internal guidance note —
  `docs/knowledgebase/CrossRepo-VIHistory.md` — repository
  `LabVIEW-Community-CI-CD/compare-vi-cli-action` — tag
  `review-template-docker-profile-requirement-v9`
- `CompareVI.Tools Release Manifest v1 schema` — internal JSON schema —
  `docs/schemas/comparevi-tools-release-manifest-v1.schema.json` — repository
  `LabVIEW-Community-CI-CD/compare-vi-cli-action` — tag
  `review-template-docker-profile-requirement-v9`
- `CompareVI.Tools VI History Capability v1 schema` — internal JSON schema —
  `docs/schemas/comparevi-tools-vi-history-capability-v1.schema.json` —
  repository `LabVIEW-Community-CI-CD/compare-vi-cli-action` — tag
  `review-template-docker-profile-requirement-v9`

## Snapshot Identification Note

- The immutable review snapshot for this requirement set is identified by the
  review tag `review-template-docker-profile-requirement-v9`.
- The exact commit identity for that tag is carried in the review packet prompt
  rather than repeated in this file, so the document does not become
  self-stale when committed.
