# Template Docker Profile Requirement V4
...
| authoritative image contract source | The Docker capability manifest value that identifies the Producer-published image contract artifact by immutable producer release pin and contract path |
...
- `TDPR-005A` When `execution_profile=docker`, the Template shall render a
  Docker capability manifest that records the authoritative image contract
  source.
...
- `TDPR-008` When `execution_profile=mixed`, the Template shall render all
  artifacts required by `TDPR-005`, `TDPR-005A`, `TDPR-006`, and `TDPR-007`.
...
- `TDPR-013` A Generated repository rendered with `execution_profile=docker` or
  ...
  image contract identified by the authoritative image contract source.
