# Template Docker Profile Requirement V3
...
| Docker workflow/documentation set | The Docker capability manifest, Docker lane policy, one or more Docker workflow scaffolds, and consumer documentation for Docker execution |
...
| pinned upstream release surface | The Producer-published CompareVI.Tools release bundle and `comparevi-tools-release.json` payload resolved by the generated capability manifest fields `authoritativeConsumerPin`, `releaseAssetName`, `releaseMetadataPath`, and `contractPaths` |
| authoritative image contract source | The Producer-published contract artifact identifier recorded by the Docker capability manifest field `producerImageContractSource` |
...
| Group | Dependency |
| --- | --- |
| Group 4: Image Contract | Depends on the authoritative image contract source published by the Producer |
| Group 5: Isolation And Host Coexistence | Depends on the supported execution model |
| Group 7: Producer / Distributor Boundary | Depends on the pinned upstream release surface published by the Producer |
| Group 8: Rendered Consumer Verification | Depends on the template self-validation matrix covering `hosted`, `docker`, and `mixed` renders |
...
- `TDPR-004` The Template shall render Docker-profile artifacts only when
...
- `TDPR-005A` When `execution_profile=docker`, the Template shall render a
  Docker capability manifest that records `producerImageContractSource`.
...
- `TDPR-013` A Generated repository rendered with `execution_profile=docker` or
  `mixed` shall consume its upstream image reference from the authoritative
  image contract source.
...
- `TDPR-025` A Generated repository rendered from the Template shall consume
  the pinned upstream release surface.
...
- `TDPR-027A` Template self-validation shall render at least one `mixed`
  consumer.
- `TDPR-028` Template self-validation shall verify that `docker` and `mixed`
  renders contain all artifacts required by `TDPR-005` through `TDPR-010`.
...
- `TDPR-031` The Docker-profile render shall preserve the required
  `vi-history` surfaces.
- `TDPR-031A` Template self-validation shall verify `TDPR-031`.
