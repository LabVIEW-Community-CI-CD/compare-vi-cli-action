...
describe('SessionIndexBuilder', () => {
    it('normalizes and sorts session index data consistently', () => {
        ...
        assert.deepStrictEqual(cases.map((testCase) => testCase.id), ['test-a', 'test-b', 'test-c']);
        ...
        assert.deepStrictEqual(index.artifacts?.map((artifact) => `${artifact.name}:${artifact.path}`), ['a-artifact:reports/a.json', 'a-artifact:reports/b.json', 'z-artifact:reports/z.json']);
