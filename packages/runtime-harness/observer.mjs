...
function normalizeWorkerCheckout(rawWorker, decision, now, adapter, repository) {
  ...
  return {
    ...
    providerId: normalizeText(rawWorker.providerId) || null,
    providerKind: normalizeText(rawWorker.providerKind) || null,
    executionPlane: normalizeText(rawWorker.executionPlane) || null,
    assignmentMode: normalizeText(rawWorker.assignmentMode) || null,
...
function normalizeWorkerReady(rawWorkerReady, decision, workerCheckout, now, adapter, repository) {
  ...
  return {
    ...
    providerId: normalizeText(rawWorkerReady.providerId) || workerCheckout?.providerId || null,
    providerKind: normalizeText(rawWorkerReady.providerKind) || workerCheckout?.providerKind || null,
    executionPlane: normalizeText(rawWorkerReady.executionPlane) || workerCheckout?.executionPlane || null,
    assignmentMode: normalizeText(rawWorkerReady.assignmentMode) || workerCheckout?.assignmentMode || null,
    issue: decision?.activeLane?.issue ?? null,
    epic: decision?.activeLane?.epic ?? null,
    forkRemote: decision?.activeLane?.forkRemote ?? null,
    branch: decision?.activeLane?.branch ?? null,
...
function normalizeWorkerBranch(rawWorkerBranch, decision, workerReady, now, adapter, repository) {
  ...
  return {
    ...
    providerId: normalizeText(rawWorkerBranch.providerId) || workerReady?.providerId || null,
    providerKind: normalizeText(rawWorkerBranch.providerKind) || workerReady?.providerKind || null,
    executionPlane: normalizeText(rawWorkerBranch.executionPlane) || workerReady?.executionPlane || null,
    assignmentMode: normalizeText(rawWorkerBranch.assignmentMode) || workerReady?.assignmentMode || null,
    issue: decision?.activeLane?.issue ?? null,
    epic: decision?.activeLane?.epic ?? null,
