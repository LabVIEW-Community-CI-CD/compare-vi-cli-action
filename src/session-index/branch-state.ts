...
import type { SessionIndexBranchState } from './schema.js';
...
function parseAheadBehind(input: string | undefined): { ahead?: number; behind?: number } {
  if (!input) {
    return {};
  }
...
export function collectBranchState(cwd: string = process.cwd()): SessionIndexBranchState | undefined {
  try {
    ...
    const state: SessionIndexBranchState = {
      branch,
      upstream,
      ahead: ahead ?? 0,
      behind: behind ?? 0,
