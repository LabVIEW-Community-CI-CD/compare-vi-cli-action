...
type OutputFormat = 'json' | 'values' | 'env' | 'sample';
...
function formatEnvOutput(index: SessionIndexV2): string {
  const lines: string[] = [];
...
function addCasesFromFiles(
  builder: ReturnType<typeof createSessionIndexBuilder>,
  caseArgs: string[] | string | undefined
): void {
  if (!caseArgs) {
    return;
  }
...
function validateSessionIndex(
  ...
  const violations = evaluateRequirements(
...
function createSampleBranchState(now: Date): SessionIndexBranchState {
  return {
    branch: 'develop',
...
try {
  let format = (args.format as OutputFormat) ?? 'json';
  if (args.sample) {
    format = 'sample';
  }
  ...
  if (format === 'sample') {
    ...
    >;
    const generatedAt =
      typeof raw.generatedAtUtc === 'string'
    ...
    if (raw.branchProtection && typeof raw.branchProtection === 'object') {
      ...
      let status: SessionIndexBranchProtection['status'] = 'ok';
      if (rawStatus === 'warn') {
        status = 'warn';
      ...
      builder.setBranchProtection({
        status,
        reason: result.reason as SessionIndexBranchProtection['reason'],
        ...
        actual:
          bp.actual && typeof bp.actual === 'object'
            ? (bp.actual as SessionIndexBranchProtection['actual'])
            : undefined,
        mapping:
  ...
  addCasesFromFiles(builder, args.cases as string[] | string | undefined);
