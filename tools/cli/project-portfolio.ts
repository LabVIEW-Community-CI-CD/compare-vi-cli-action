...
function main(): void {
  ...
  const report = {
    ...
    project: {
      ...
      repositories: [...new Set(normalizedItems.map((item) => item.repository).filter((value): value is string => Boolean(value)))].sort(),
    },
  ...
  // eslint-disable-next-line no-console
  console.log(
    `${statusLabel} Items expected=${config.items.length} actual=${normalizedItems.length} drift=${drift.ok ? 'none' : 'present'}`,
  );
