...
test('handoff entrypoint contract is wired into automation and operator docs', () => {
  ...
  const controlPlaneRefresh = readText('tools/priority/Refresh-HandoffControlPlane.ps1');
  ...
  assert.match(printHandoff, /Refresh-HandoffControlPlane\.ps1/);
  assert.match(controlPlaneRefresh, /handoff-monitoring-mode\.mjs/);
  assert.match(controlPlaneRefresh, /monitoring-mode\.json/);
  assert.match(controlPlaneRefresh, /release-published-bundle-observer\.mjs/);
  assert.match(controlPlaneRefresh, /release-published-bundle-observer\.json/);
  assert.match(controlPlaneRefresh, /release-signing-readiness\.mjs/);
  assert.match(controlPlaneRefresh, /release-signing-readiness\.json/);
  assert.match(controlPlaneRefresh, /autonomous-governor-summary\.mjs/);
  assert.match(controlPlaneRefresh, /autonomous-governor-summary\.json/);
  assert.match(controlPlaneRefresh, /autonomous-governor-portfolio-summary\.mjs/);
  assert.match(controlPlaneRefresh, /autonomous-governor-portfolio-summary\.json/);
  assert.match(controlPlaneRefresh, /sagan-context-concentrator\.mjs/);
  assert.match(controlPlaneRefresh, /sagan-context-concentrator\.json/);
