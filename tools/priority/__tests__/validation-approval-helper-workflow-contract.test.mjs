...
test('validation approval helper workflow downloads source artifacts and uploads the decision artifacts', () => {
  ...
  assert.match(workflow, /name: Download existing broker decision artifact\s+if: inputs\.broker_mode == 'consume'\s+uses: actions\/download-artifact@v8/);
  assert.match(workflow, /name: Download validation attestation artifact\s+if: inputs\.broker_mode == 'evaluate'\s+uses: actions\/download-artifact@v8/);
  assert.match(workflow, /name: Download validation deployment determinism artifact\s+if: inputs\.broker_mode == 'evaluate'\s+uses: actions\/download-artifact@v8/);
  assert.match(workflow, /run-id: \$\{\{ inputs\.artifact_run_id \|\| inputs\.run_id \}\}/);
  assert.match(workflow, /name: Upload validation approval artifacts\s+if: always\(\)\s+uses: actions\/upload-artifact@v7/);
