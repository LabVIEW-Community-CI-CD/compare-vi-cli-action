...
test('runTemplatePivotGate treats queue-empty as authoritative even when open excluded inventory remains', async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'template-pivot-gate-queue-empty-inventory-'));
  const policyPath = path.join(tmpDir, 'template-pivot-gate.json');
  const queueEmptyReportPath = path.join(tmpDir, 'no-standing-priority.json');
  const releaseSummaryPath = path.join(tmpDir, 'release-summary.json');
  const handoffEntrypointStatusPath = path.join(tmpDir, 'entrypoint-status.json');
  ...
  writeJson(queueEmptyReportPath, {
    schema: 'standing-priority/no-standing@v1',
    message: 'queue empty with standing-excluded inventory',
    reason: 'queue-empty',
    openIssueCount: 10
  });
  writeJson(releaseSummaryPath, {
  ...
  writeJson(templateAgentVerificationReportPath, {
    ...
    iteration: {
      label: 'post-merge queue-empty proof',
      ref: 'issue/origin-1783-pivot-gate-queue-empty',
    ...
    provenance: {
      ...
      execution: {
        executionPlane: 'linux-tools-image',
        containerImage: 'ghcr.io/labview-community-ci-cd/comparevi-tools:v0.1.0',
        generatedConsumerWorkspaceRoot: 'E:\\comparevi-template-consumers\\run-queue-empty',
        laneId: 'lane-template-verify',
        agentId: 'sagan',
        fundingWindowId: 'HQ1VJLMV-0027'
      }
    ...
    metrics: {
      targetSlotCount: 8,
      reservedSlotCount: 1,
      implementationSlotsRemaining: 7,
      durationWithinGoal: true,
      recommendationPresent: true
    },
    blockers: []
  ...
  assert.equal(report.summary.status, 'ready');
  assert.equal(report.summary.readyForFutureAgentPivot, true);
  assert.equal(report.evidence.queueEmpty.ready, true);
  assert.equal(report.evidence.queueEmpty.openIssueCount, 10);
  assert.ok(report.summary.blockers.every((entry) => entry.code !== 'open-issues-remaining'));
  assert.equal(fs.existsSync(outputPath), true);
});
