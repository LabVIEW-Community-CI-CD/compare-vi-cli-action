...
test('release workflow resolves downloaded artifacts through the shared helper before validation', () => {
  ...
  const downstreamRunIndex = releaseContractSteps.findIndex((step) => step?.name === 'Resolve latest downstream proving run');
  const downstreamDownloadIndex = releaseContractSteps.findIndex((step) => step?.name === 'Download downstream proving artifact');
  const downstreamResolveIndex = releaseContractSteps.findIndex((step) => step?.name === 'Resolve downstream proving artifact paths');
  ...
  assert.ok(downstreamRunIndex >= 0, 'release-contract should resolve the latest downstream proving run');
  assert.ok(downstreamDownloadIndex > downstreamRunIndex, 'release-contract should download downstream proving artifacts after run resolution');
  assert.ok(downstreamResolveIndex > downstreamDownloadIndex, 'release-contract should resolve downstream proving artifact paths after download');
  ...
  assert.match(workflowRaw, /name: Resolve latest downstream proving run/);
  assert.match(workflowRaw, /downstream-onboarding-feedback\.yml\/runs\?branch=develop&status=success/);
  assert.match(workflowRaw, /name: Download downstream proving artifact/);
  assert.match(workflowRaw, /download-run-artifact\.mjs/);
  assert.match(workflowRaw, /--artifact downstream-onboarding-feedback/);
  assert.match(workflowRaw, /name: Resolve downstream proving artifact paths/);
  assert.match(workflowRaw, /steps\.downstream_proving_artifacts\.outputs\.scorecard_path/);
  ...
  assert.match(workflowRaw, /--downstream-promotion "\$\{\{\s*steps\.downstream_proving_artifacts\.outputs\.scorecard_path\s*\}\}"/);
