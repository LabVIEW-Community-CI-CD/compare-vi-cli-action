...
test('agent cost telemetry knowledgebase points at the checked-in precursor surfaces instead of hidden billing assumptions', () => {
  ...
  assert.match(guide, /Operator Labor And Blended Autonomy Cost/);
  assert.match(guide, /docs\/schemas\/operator-cost-profile-v1\.schema\.json/);
  assert.match(guide, /tools\/policy\/operator-cost-profile\.json/);
  assert.match(guide, /laborRateUsdPerHour = 250/);
  assert.match(guide, /total autonomy cost = `operatorLaborUsd \+ tokenCostUsd`/);
  assert.match(guide, /one agent running for one hour = `\$250` operator labor before token cost/);
  assert.match(guide, /pricingBasis = agent-runtime-hour/);
  ...
  assert.match(guide, /rollingAverageBlendedUsdPerIssue/);
