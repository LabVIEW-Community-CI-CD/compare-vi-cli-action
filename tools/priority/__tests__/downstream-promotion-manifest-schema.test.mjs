...
test('downstream promotion manifest omits the optional LV32 receipt when absent and still validates schema', async () => {
  const schemaPath = path.join(repoRoot, 'docs', 'schemas', 'downstream-promotion-manifest-v1.schema.json');
  const schema = JSON.parse(await readFile(schemaPath, 'utf8'));

  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'downstream-promotion-schema-no-receipt-'));
  const policyPath = path.join(tmpDir, 'downstream-promotion-contract.json');
  writeJson(policyPath, {
    schema: 'priority/downstream-promotion-contract@v1',
    sourceRef: 'upstream/develop',
  ...
  assert.equal('viHistoryLv32ShadowProofReceipt' in manifest.inputs, false);
