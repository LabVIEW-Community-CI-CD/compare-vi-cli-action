...
test('pester evidence distinguishes context-blocked, selection-blocked, and readiness-blocked skips from seam defects', () => {
  ...
  assert.match(workflow, /\$classification = 'results-xml-truncated'/);
  assert.match(workflow, /\$classification = 'invalid-results-xml'/);
  assert.match(workflow, /\$classification = 'missing-results-xml'/);
  assert.match(workflow, /execution-receipt-results-xml-truncated/);
  assert.match(workflow, /execution-receipt-invalid-results-xml/);
  assert.match(workflow, /execution-receipt-missing-results-xml/);
  assert.match(workflow, /results-xml-status=/);
  ...
  assert.match(workflow, /\$executionReceiptStatus -eq 'results-xml-truncated'/);
  assert.match(workflow, /\$executionReceiptStatus -eq 'invalid-results-xml'/);
  assert.match(workflow, /\$executionReceiptStatus -eq 'missing-results-xml'/);
