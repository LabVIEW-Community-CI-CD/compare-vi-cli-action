...
test('pester selection owns pack shaping and dispatcher profile resolution before execution begins', () => {
  ...
  assert.match(workflow, /execution_pack:/);
  assert.match(workflow, /Normalize include_integration/);
  assert.match(workflow, /Resolve execution pack/);
  assert.match(workflow, /Resolve-PesterExecutionPack/);
  assert.match(workflow, /Resolve dispatcher profile/);
  assert.match(workflow, /pester-selection-receipt@v1/);
  assert.match(workflow, /executionPack/);
  assert.match(workflow, /refineIncludePatterns/);
  assert.match(workflow, /effectiveIncludePatterns/);
...
test('pester run is execution-only and validates context, readiness, and selection receipts before dispatch', () => {
  ...
  assert.match(workflow, /execution_pack=/);
  assert.match(workflow, /refine_include_patterns_json=/);
  assert.match(workflow, /selection-blocked/);
  assert.match(workflow, /Run Pester tests via local dispatcher/);
  assert.match(workflow, /\$bound\.ExecutionPack = \$executionPack/);
  assert.match(workflow, /Postprocess execution results/);
  assert.match(workflow, /Invoke-PesterExecutionPostprocess\.ps1/);
  assert.match(workflow, /Materialize execution telemetry/);
  assert.match(workflow, /Invoke-PesterExecutionTelemetry\.ps1/);
  assert.match(workflow, /\$ErrorActionPreference = 'Continue'/);
  assert.match(workflow, /\$stepOutputPath = \$env:GITHUB_OUTPUT/);
  assert.match(workflow, /dispatcher-github-output\.txt/);
  assert.match(workflow, /"exit_code=\$exitCode"/);
  assert.match(workflow, /Write-Warning \("Dispatcher exited with code \{0\}" -f \$exitCode\)/);
  assert.match(workflow, /if \(\$dispatcherExitCode -eq '' -and \(Test-Path -LiteralPath \$dispatcherOutputTrace\)\)/);
  assert.match(workflow, /results-xml-truncated/);
  assert.match(workflow, /invalid-results-xml/);
  assert.match(workflow, /missing-results-xml/);
  assert.match(workflow, /unsupported-schema/);
  assert.match(workflow, /selectionExecutionPack/);
  assert.match(workflow, /telemetryStatus/);
  assert.match(workflow, /telemetryLastKnownPhase/);
  assert.match(workflow, /telemetryEventCount/);
  assert.match(workflow, /pester-execution-telemetry\.json/);
  ...
  assert.doesNotMatch(workflow, /Export workflow token for priority sync/);
...
test('pester evidence distinguishes context-blocked, selection-blocked, and readiness-blocked skips from seam defects', () => {
  ...
  const classificationTool = readRepoFile('tools/Invoke-PesterEvidenceClassification.ps1');
  const totalsTool = readRepoFile('tools/Write-PesterTotals.ps1');
  ...
  assert.match(workflow, /name:\s+\$\{\{\s*steps\.artifact_name\.outputs\.name\s*\}\}\s*\n\s+path:\s+tests\/results/);
  assert.match(workflow, /Validate execution receipt artifact/);
  assert.match(workflow, /PesterServiceModelSchema\.ps1/);
  assert.match(workflow, /status=unsupported-schema/);
  assert.match(workflow, /execution_pack=/);
  assert.match(workflow, /Write-PesterSummaryToStepSummary\.ps1/);
  assert.match(workflow, /Ensure-SessionIndex\.ps1/);
  assert.match(workflow, /Write-PesterTotals\.ps1/);
  assert.match(workflow, /Invoke-PesterEvidenceClassification\.ps1/);
  assert.match(workflow, /Invoke-PesterOperatorOutcome\.ps1/);
  ...
  assert.match(workflow, /classification=\$\{\{\s*steps\.classify\.outputs\.classification\s*\}\}/);
  assert.match(workflow, /next_action=\$\{\{\s*steps\.operator_outcome\.outputs\.next_action\s*\}\}/);
  assert.match(workflow, /pester-operator-outcome\.json/);
  assert.match(workflow, /pester-evidence-classification\.json/);
  assert.match(workflow, /Invoke-PesterEvidenceProvenance\.ps1/);
  assert.match(workflow, /pester-evidence-provenance\.json/);
  assert.match(classificationTool, /execution-receipt-missing/);
  assert.match(classificationTool, /classification = 'seam-defect'/);
  assert.match(classificationTool, /function Get-OptionalStringProperty/);
  assert.match(classificationTool, /selectionExecutionPack = Get-OptionalStringProperty/);
  assert.match(classificationTool, /\$classification = 'context-blocked'/);
  assert.match(classificationTool, /\$classification = 'selection-blocked'/);
  assert.match(classificationTool, /\$classification = 'readiness-blocked'/);
  assert.match(classificationTool, /\$classification = 'results-xml-truncated'/);
  assert.match(classificationTool, /\$classification = 'invalid-results-xml'/);
  assert.match(classificationTool, /\$classification = 'missing-results-xml'/);
  assert.match(classificationTool, /\$classification = 'unsupported-schema'/);
  assert.match(classificationTool, /execution-job-unsupported-schema/);
  assert.match(classificationTool, /execution-receipt-unsupported-schema/);
  assert.match(classificationTool, /execution-receipt-results-xml-truncated/);
  assert.match(classificationTool, /execution-receipt-invalid-results-xml/);
  assert.match(classificationTool, /execution-receipt-missing-results-xml/);
  assert.match(classificationTool, /results-xml-status=/);
  assert.match(classificationTool, /\$effectiveContextStatus -ne 'ready'/);
  assert.match(classificationTool, /\$effectiveSelectionStatus -ne 'ready'/);
  assert.match(classificationTool, /\$executionReceiptStatus -eq 'selection-blocked'/);
  assert.match(classificationTool, /\$executionReceiptStatus -eq 'context-blocked'/);
  assert.match(classificationTool, /\$executionReceiptStatus -eq 'results-xml-truncated'/);
  assert.match(classificationTool, /\$executionReceiptStatus -eq 'invalid-results-xml'/);
  assert.match(classificationTool, /\$executionReceiptStatus -eq 'missing-results-xml'/);
  assert.match(classificationTool, /\$effectiveReadinessStatus -ne 'ready' -and \$effectiveExecutionJobResult -in @\('skipped', 'cancelled'\)/);
  assert.match(classificationTool, /raw-artifact-download=/);
  assert.match(classificationTool, /execution-receipt-seam-defect/);
  assert.match(totalsTool, /schema = 'pester-totals\/v1'/);
  assert.match(totalsTool, /status = 'missing-summary'/);
});
