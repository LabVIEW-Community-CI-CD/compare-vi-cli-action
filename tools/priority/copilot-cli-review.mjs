...
export async function loadCopilotCliReviewPolicy(repoRoot) {
  try {
    ...
    return normalizeCopilotCliReviewPolicy(raw?.localReviewLoop?.copilotCliReview ?? {});
  } catch (error) {
...
export function resolveRepoGitState(repoRoot) {
  ...
  const mergeBase = runGit(repoRoot, ['merge-base', 'HEAD', 'upstream/develop']);
  const dirty = runGit(repoRoot, ['status', '--short', '--untracked-files=no']);
  return {
...
function resolveMergeBase(repoRoot, env = process.env) {
  ...
  if (baseRef && env.GITHUB_ACTIONS === 'true') {
    for (const remote of ['origin', 'upstream']) {
      const fetchResult = runGit(repoRoot, ['fetch', '--no-tags', '--prune', '--depth=1', remote, baseRef], env);
      if (fetchResult.status !== 0) {
        continue;
      }
      for (const candidate of [
...
export function collectReviewContext({
  ...
  const gitState = resolveRepoGitState(repoRoot);
  ...
  if (normalizedMode === 'staged') {
    changedFiles = stagedFiles.length > 0
      ? stagedFiles
      : normalizeStringList(runGit(repoRoot, ['diff', '--cached', '--name-only', '--diff-filter=ACMRT']).stdout);
    diffArgs = ['diff', '--cached', '--unified=3', '--no-ext-diff'];
  } else {
    baseRef = resolveMergeBase(repoRoot);
    if (!baseRef) {
      throw new Error('Unable to resolve a merge base for Copilot CLI review.');
    }
    changedFiles = normalizeStringList(runGit(repoRoot, ['diff', '--name-only', '--diff-filter=ACMRT', `${baseRef}..HEAD`]).stdout);
  ...
  const diffResult = runGit(repoRoot, [...diffArgs, '--', ...selectedFiles]);
