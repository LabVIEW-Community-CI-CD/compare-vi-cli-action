...
while ($true) {
  ...
  } elseif ($dispatcher.status -eq 'completed') {
    break
  } else {
    Write-Host ("Dispatcher job status: {0} (conclusion={1}) – polling again in {2}s" -f $dispatcher.status, $dispatcher.conclusion, $PollSeconds) -ForegroundColor Yellow
  }
